...
static const char kVertexShaderSkinned[] =
    ...
    "}\n"
    "\n"
    "vec4 GetSkinnedPosition(vec4 position, int bone_index) {\n"
    "  return position * BoneTransformMatrixTranspose(bone_index);\n"
    "}\n"
    "\n"
    "vec4 FourBoneSkinnedPosition(vec4 position) {\n"
    "  vec4 p0 = GetSkinnedPosition(position, int(aBoneIndices.x));\n"
    "  vec4 p1 = GetSkinnedPosition(position, int(aBoneIndices.y));\n"
    "  vec4 p2 = GetSkinnedPosition(position, int(aBoneIndices.z));\n"
    "  vec4 p3 = GetSkinnedPosition(position, int(aBoneIndices.w));\n"
    "  return p0*aBoneWeights.x\n"
    "       + p1*aBoneWeights.y\n"
    "       + p2*aBoneWeights.z\n"
    "       + p3*aBoneWeights.w;\n"
    "}\n"
    ...
    "void main()\n"
    "{\n"
    "  vec4 model_position = FourBoneSkinnedPosition(aPosition);\n"
